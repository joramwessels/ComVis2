...
void VoxelReconstruction::run(int argc, char** argv)
{
	...
	std::vector<Reconstructor::Voxel*> reconVoxels = reconstructor.getVoxels();
	cv::Mat voxels = cv::Mat(reconVoxels.size(), 3, CV_32F);
	for (int i = 0; i < reconVoxels.size(); i++)
	...
	cv::Mat labels = cv::Mat(reconVoxels.size(), 1, CV_32S);
